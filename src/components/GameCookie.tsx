...
export function GameCookie() {
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div className="my-8 flex flex-col items-center" style={{ position: 'absolute', top: '40px', zIndex: 2 }}>
      ...
      <div className="my-8" style={{ position: 'absolute', left: '35%' }}>
        <h2 className="text-center text-3xl font-semibold text-green-500 md:text-5xl"style={{ fontSize: '1.5rem' }}>{game?.cash}</h2>
        <div className="my-8" style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}>
          <h2 className="text-center text-3xl font-semibold text-blue-900 md:text-5xl" style={{ fontSize: '1.5rem' }}>{data}</h2>
        </div>
      </div>
      {isAnimationVisible && (
