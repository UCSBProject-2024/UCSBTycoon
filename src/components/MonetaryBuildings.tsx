...
const MonetaryBuildings: React.FC = () => {
  ...
  const handleClick = (buildingName: string) => () => {
    if (!clickedButtons.has(buildingName)|| clickedButtons.has(buildingName)) {
      console.log('Building clicked:', buildingName);

      // Mark the building as bought in local storage
      if (buildingName === 'The Club & Guest House' && game?.cash >= 100 && game?.knowledge >= 50) {
      ...
      if (buildingName === 'University Center(UCEN)' && game?.cash >= 500 && game?.knowledge >= 250) {
        localStorage.setItem(buildingName, 'true');
        gameDispatch({ type: 'update', subtype: 'cashMult' }); //sends building name to backend
        console.log('increment cash');
        gameDispatch({ type: 'update', subtype: 'setCash', cash: 500});
      }
      if (buildingName === 'De La Guerra Dining Hall' && game?.cash >= 3000 && game?.knowledge >= 1500) {
        localStorage.setItem(buildingName, 'true');
        gameDispatch({ type: 'update', subtype: 'cashMult' }); //sends building name to backend
        console.log('increment cash');
        gameDispatch({ type: 'update', subtype: 'setCash', cash: 3000});
      }
      if (buildingName === 'San Miguel Residence Hall' && game?.cash >= 25000 && game?.knowledge >= 12500) {
        localStorage.setItem(buildingName, 'true');
        gameDispatch({ type: 'update', subtype: 'cashMult' }); //sends building name to backend
        console.log('increment cash');
        gameDispatch({ type: 'update', subtype: 'setCash', cash: 25000});
      }
      if (buildingName === 'San Nicholas Residence Hall' && game?.cash >= 200000 && game?.knowledge >= 100000) {
        localStorage.setItem(buildingName, 'true');
        gameDispatch({ type: 'update', subtype: 'cashMult' }); //sends building name to backend
        console.log('increment cash');
        gameDispatch({ type: 'update', subtype: 'setCash', cash: 200000});
      }
      if (buildingName === 'Santa Cruz Residence Hall' && game?.cash >= 1000000 && game?.knowledge >= 500000) {
        localStorage.setItem(buildingName, 'true');
        gameDispatch({ type: 'update', subtype: 'cashMult' }); //sends building name to backend
        console.log('increment cash');
        gameDispatch({ type: 'update', subtype: 'setCash', cash: 1000000});
      }
      if (buildingName === 'Anacapa Residence Hall' && game?.cash >= 5000000 && game?.knowledge >= 2500000) {
        localStorage.setItem(buildingName, 'true');
        gameDispatch({ type: 'update', subtype: 'cashMult' }); //sends building name to backend
        console.log('increment cash');
        gameDispatch({ type: 'update', subtype: 'setCash', cash: 5000000});
      }
      if (buildingName === 'Carrillo Dining Hall' && game?.cash >= 15000000  && game?.knowledge >= 7500000) {
        localStorage.setItem(buildingName, 'true');
        gameDispatch({ type: 'update', subtype: 'cashMult' }); //sends building name to backend
        console.log('increment cash');
        gameDispatch({ type: 'update', subtype: 'setCash', cash: 15000000});
      }
      if (buildingName === 'Ortega Dining Hall' && game?.cash >= 500000000 && game?.knowledge >= 250000000) {
        localStorage.setItem(buildingName, 'true');
        gameDispatch({ type: 'update', subtype: 'cashMult' }); //sends building name to backend
        console.log('increment cash');
      ...
      //knowledgeBuildings
      if (buildingName === 'Library' && game?.knowledge >= 100 && game?.cash >=100) {
        localStorage.setItem(buildingName, 'true');
        ...
        return (
          <div key={index}>
            ...
            >
              COST: {building.cost}
              <p> </p>
              KNOWLEDGE REQUIRED: {building.knowledge}
              <p> </p>
              Name: {building.name}
              <p> </p>
